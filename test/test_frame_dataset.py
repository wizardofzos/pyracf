...
def test_frame_datasets_acl_resolve(testparms):
  r = testparms['object']
  t1 = r.datasets.find('SYS1.**').acl(explode=True)
  t2 = r.datasets.find('SYS1.**').acl(resolve=True)
...
def test_frame_datasets_acl_admin(testparms):
  r = testparms['object']
  t1 = r.datasets.find('SYS1.**').acl(admin=True)
