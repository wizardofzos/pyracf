...
def test_frame_datasets(testparms):
  r = testparms['object']
  t1 = r.datasets.pick('SYS1.**')
  t2 = t1.skip(OWNER_ID='SYS1')
  assert t1.shape[0]>1, 'datasets.pick must select several profiles'
  assert t1.shape[0]>=t2.shape[0], 'skip() is supposed to reduce the number of rows'

def test_frame_datasets_acl(testparms):
  r = testparms['object']
  t1 = r.datasets.pick('SYS1.**')
  t2 = r.datasets.pick('SYS1.**').acl()
...
def test_frame_datasets_acl_resolve(testparms):
  r = testparms['object']
  t1 = r.datasets.pick('SYS1.**').acl(explode=True)
  t2 = r.datasets.pick('SYS1.**').acl(resolve=True)
...
def test_frame_datasets_acl_admin(testparms):
  r = testparms['object']
  t1 = r.datasets.pick('SYS1.**').acl(admin=True)
